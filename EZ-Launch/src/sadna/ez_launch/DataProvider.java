...
public class DataProvider extends ContentProvider {
	...
	public static final String TAG = "sadna.DataProvider";

	private static final String AUTHORITY_BASE = "sadna.ez_launch";
	...
	public enum DataProviderColumns {
		...
		_id, image, name, launchuri
	}
	
	public static final String[] PROJECTION_APPWIDGETS = new String[] { DataProviderColumns._id.toString(),
		DataProviderColumns.image.toString(), DataProviderColumns.name.toString(), DataProviderColumns.launchuri.toString()};
	...
	public static ExtMatrixCursor loadNewData(ContentProvider mcp, String[] projection, long GroupId, int NameKind, int clickActn) {
		...
		//relevant to contacts ... we need todo something else for applications...
		...
		try
		{
			while (!cur.isAfterLast()) {
				...
				for (int i = 0, count = projection.length; i < count; i++) {
					...
					} else if (DataProviderColumns.image.toString().equals(column)) {
						values[i] = getImg(id);
					} else if (DataProviderColumns.launchuri.toString().equals(column)) {
						if (withPhone) {
