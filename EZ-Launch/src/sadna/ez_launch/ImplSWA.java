...
import tools.ez_launch.BoundRemoteViews;
import tools.ez_launch.LauncherIntent;
import tools.ez_launch.SimpleRemoteViews;

public class ImplSWA implements WidgetImplementation {
	...
	public Intent CreateMakeScrollableIntent(Context context, int appWidgetId) {
		...
		itemViews.setBoundBitmap(R.id.photo, "setImageBitmap",
				DataProvider.DataProviderColumns.image.ordinal(), R.drawable.no_image);

		if (autosizeImages) {
			int width = ShortcutWidget.calcWidthPixel(context, appWidgetId, mWidget.getWidth()); // get the widget width in dip
		...
		itemViews.SetBoundOnClickIntent(R.id.photo, pendingIntent,
				...
				DataProvider.DataProviderColumns.launchuri.ordinal());
		if (textVisibility == View.VISIBLE) {
	...
	public void onAppWidgetReady(Context context, Intent intent) {
		...
		if (APIVersion < 2) {
			...
			RemoteViews views = new RemoteViews(context.getPackageName(), isICS ? R.layout.main_swa_ics : R.layout.main);
			views.setTextViewText(R.id.loading, "launcher is not supported, too old");
	        if (Preferences.getBGImage(context, appWidgetId) == Preferences.BG_BLACK) {
